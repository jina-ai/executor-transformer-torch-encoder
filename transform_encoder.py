...
class TransformerTorchEncoder(Executor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if device == 'cuda' and not torch.cuda.is_available():
            logger.warning('You tried to use GPU but torch did not detect your'
    ...
    def _get_docs_batch_generator(self, docs: DocumentArray, parameters: Dict):
        traversal_path = parameters.get('traversal_path',
                                        self.default_traversal_path)
        batch_size = parameters.get('batch_size', self.default_batch_size)
        flat_docs = docs.traverse_flat(traversal_path)
        filtered_docs = DocumentArray([doc for doc in flat_docs if
                         doc is not None and doc.text is not None])
